...
Le centre `/dashboard/automatisations` gère les modèles, séquences multi-étapes, e-mails automatiques ou manuels, appels, tâches, délais, jours ouvrés, fenêtres d’envoi, fuseaux, inscriptions consenties, règles déclenchées par les événements CRM et journal d’envoi. Une bibliothèque pisciniste installe des brouillons relisibles, jamais des règles actives à l’insu de l’équipe. Une étape manuelle peut bloquer la suite jusqu’à sa réalisation dans Organisation. Chaque inscription peut être mise en pause, reprise ou arrêtée individuellement, avec prochaine échéance et motif visibles. Chaque étape expose ses volumes livrés, ouverts, cliqués, en erreur ou ses tâches terminées.

Les e-mails ajoutent un lien de désinscription signé et un en-tête `List-Unsubscribe`. Les rejets permanents, plaintes et suppressions fournisseur alimentent une liste de suppression propre à la société, bloquent tout nouvel envoi et arrêtent les séquences concernées. Le journal indique les tentatives et la prochaine reprise ; après épuisement de la politique de retry, l’envoi passe dans une file de reprise manuelle. Google Workspace et Microsoft 365 créent d’abord un brouillon fournisseur dont l’identifiant est persisté avant l’envoi, afin qu’une reprise après interruption retrouve le message au lieu de le dupliquer. Resend conserve en plus sa clé d’idempotence.

Le centre Campagnes rattache une liste active ou statique à une séquence active. Le lancement contrôle côté serveur l’adresse, le consentement, l’opposition, le statut du prospect et les inscriptions déjà présentes avant de créer le lot. Les audiences supérieures à 5 000 membres doivent être découpées afin de garder une diffusion observable et maîtrisée.

Le worker traite les échéances chaque minute. Les étapes manuelles fonctionnent même sans fournisseur e-mail ; les étapes d’envoi utilisent la messagerie active la plus récemment configurée (Resend, Google Workspace ou Microsoft 365). Le composeur manuel permet de choisir explicitement la boîte. Chaque processeur prend un bail PostgreSQL global avant de travailler : deux workers ou deux appels de cron concurrents ne traitent donc pas le même lot. Le workflow GitHub Actions `production-processors.yml` appelle les trois processeurs toutes les cinq minutes sur l’hébergement Vercel Hobby et accepte aussi un déclenchement manuel. Son secret `PROCESSOR_CRON_SECRET` doit contenir la même valeur que `AUTOMATION_CRON_SECRET` en production.

Dans `/dashboard/communications`, chaque entreprise peut fournir ses propres clés Resend ou autoriser sa boîte Google/Microsoft par OAuth avec PKCE, sans transmettre son mot de passe. Les secrets, jetons et curseurs incrémentaux sont chiffrés au repos, l’adresse réellement autorisée est vérifiée et la synchronisation entrante/sortante des messages et événements calendrier peut être déclenchée manuellement ou par `GET|POST /api/communications/sync/process` avec `COMMUNICATIONS_CRON_SECRET` (repli sur `AUTOMATION_CRON_SECRET`). Les tâches Organisation peuvent être créées ou supprimées dans le calendrier autorisé ; les événements distants et leurs annulations sont importés de façon idempotente. Pour Resend, la clé doit permettre l’accès complet si la réception est utilisée ; une clé limitée à l’envoi suffit sinon. Le webhook signé reste `/api/webhooks/resend`.
...
Le même worker planifie toutes les cinq minutes les visites d’entretien, les factures récurrentes et les relances de factures arrivées à échéance. Les relances se configurent dans Paramètres, utilisent la messagerie active, sont journalisées dans la facture et dans Communications et portent une clé de palier persistante. Une activation tardive ne produit qu’un rappel adapté par facture, jamais une rafale de rattrapage. Un ordonnanceur externe peut appeler `POST /api/scheduling/process` avec `SCHEDULER_CRON_SECRET`, ou `AUTOMATION_CRON_SECRET` si aucun secret distinct n’est défini.
