...
export async function getAutomationDashboard() {
  return withAuth(async ({ companyId }) => {
    ...
    const [templates, sequences, workflows, deliveries, leads, clients, deliveryStats, runStats, emailChannel] = await Promise.all([
    ...
    return {
      templates: templates.map((template) => ({ id: template.id, name: template.name, category: template.category, subject: template.subject, bodyHtml: template.bodyHtml, status: template.status, updatedAt: template.updatedAt.toISOString() })),
      sequences: sequences.map((sequence) => ({
        ...
        steps: sequence.steps.map((step) => ({ id: step.id, position: step.position, delayHours: step.delayHours, type: step.type, subject: step.subject, bodyHtml: step.bodyHtml, taskTitle: step.taskTitle, taskNotes: step.taskNotes, taskPriority: step.taskPriority, pauseUntilComplete: step.pauseUntilComplete, deliveries: step.deliveries, taskExecutions: step.taskExecutions.map((execution) => ({ completedAt: execution.completedAt?.toISOString() ?? null, organisationTask: execution.organisationTask })) })),
        enrollments: sequence.enrollments.map((enrollment) => ({ id: enrollment.id, status: enrollment.status, nextStepPosition: enrollment.nextStepPosition, nextSendAt: enrollment.nextSendAt?.toISOString() ?? null, lastSentAt: enrollment.lastSentAt?.toISOString() ?? null, stopReason: enrollment.stopReason, enrolledAt: enrollment.enrolledAt.toISOString(), completedAt: enrollment.completedAt?.toISOString() ?? null, leadCapture: enrollment.leadCapture, taskExecutions: enrollment.taskExecutions.map((execution) => ({ completedAt: execution.completedAt?.toISOString() ?? null, step: execution.step, organisationTask: execution.organisationTask })) })),
      })),
      workflows: workflows.map((workflow) => ({
        ...
        runs: workflow.runs.map((run) => ({ id: run.id, event: run.event, subjectModel: run.subjectModel, subjectId: run.subjectId, status: run.status, output: run.output, error: run.error, startedAt: run.startedAt.toISOString(), completedAt: run.completedAt?.toISOString() ?? null })),
        versions: workflow.versions.map((version) => ({ id: version.id, version: version.version, status: version.status, trigger: version.trigger, publishedAt: version.publishedAt?.toISOString() ?? null, createdAt: version.createdAt.toISOString() })),
      })),
      deliveries: deliveries.map((delivery) => ({ id: delivery.id, recipientEmail: delivery.recipientEmail, subject: delivery.subject, status: delivery.status, error: delivery.error, scheduledAt: delivery.scheduledAt.toISOString(), sentAt: delivery.sentAt?.toISOString() ?? null, createdAt: delivery.createdAt.toISOString(), sequence: delivery.sequence })),
      leads,
      clients: clients.map((client) => ({ id: client.id, name: client.name, score: client.relationScore, status: customerHealthStatus(client.relationScore), previousScore: client.healthSnapshots[1]?.score ?? client.healthSnapshots[0]?.score ?? null })),
      stats: {
      ...
      readiness: {
        emailProviderConfigured: Boolean(process.env.RESEND_API_KEY?.trim() && process.env.EMAIL_FROM?.trim()),
...
export async function duplicateEmailSequence(sequenceId: string) {
  return withAuth(async ({ companyId, userId }) => {
    ...
    const name = nextCopyName(source.name, names.map((item) => item.name))
    const copy = await prisma.emailSequence.create({
      data: {
        ...
        steps: { create: source.steps.map((step) => ({ position: step.position, delayHours: step.delayHours, type: step.type, subject: step.subject, bodyHtml: step.bodyHtml, taskTitle: step.taskTitle, taskNotes: step.taskNotes, taskPriority: step.taskPriority, pauseUntilComplete: step.pauseUntilComplete })) },
      },
    })
...
export async function addEmailSequenceStep(input: unknown) {
  return withAuth(async ({ companyId, userId }) => {
    ...
    const step = await prisma.emailSequenceStep.create({ data: { sequenceId: sequence.id, position: (sequence.steps[0]?.position ?? -1) + 1, delayHours: data.delayHours, type: data.type, subject: subject || "", bodyHtml: bodyHtml || "", taskTitle: data.type === "EMAIL" ? null : data.taskTitle || null, taskNotes: data.type === "EMAIL" ? null : data.taskNotes || null, taskPriority: data.taskPriority, pauseUntilComplete: data.type === "EMAIL" ? false : data.pauseUntilComplete } })
    await logAction({ userId, action: "CREATE_EMAIL_SEQUENCE_STEP", resource: "EMAIL_SEQUENCE", resourceId: sequence.id, payload: { stepId: step.id, type: step.type, position: step.position } })
    revalidatePath("/dashboard/automatisations")
    return { success: true as const }
  }, "automation.write")
}
...
export async function deleteEmailSequenceStep(stepId: string) {
  return withAuth(async ({ companyId, userId }) => {
    ...
    await prisma.$transaction(async (tx) => {
      ...
      const following = await tx.emailSequenceStep.findMany({ where: { sequenceId: step.sequenceId, position: { gt: step.position } }, select: { id: true, position: true }, orderBy: { position: "asc" } })
      for (const item of following) await tx.emailSequenceStep.update({ where: { id: item.id }, data: { position: item.position - 1 } })
    })
    await logAction({ userId, action: "DELETE_EMAIL_SEQUENCE_STEP", resource: "EMAIL_SEQUENCE", resourceId: step.sequenceId, payload: { stepId: step.id, position: step.position } })
    revalidatePath("/dashboard/automatisations")
    return { success: true as const }
  }, "automation.write")
}
...
export async function updateEmailSequenceStatus(sequenceId: string, status: string) {
  return withAuth(async ({ companyId, userId }) => {
    ...
    const sequence = await prisma.emailSequence.findFirst({ where: { id, companyId }, include: { _count: { select: { steps: true, enrollments: { where: { status: { in: ["ACTIVE", "PAUSED"] } } } } } } })
    if (!sequence) throw new Error("Séquence introuvable")
...
export async function updateEmailSequenceSettings(input: unknown) {
  return withAuth(async ({ companyId, userId }) => {
    ...
    await logAction({ userId, action: "UPDATE_EMAIL_SEQUENCE_SETTINGS", resource: "EMAIL_SEQUENCE", resourceId: sequence.id, payload: { businessDaysOnly: data.businessDaysOnly, sendWindowStart: data.sendWindowStart, sendWindowEnd: data.sendWindowEnd, timezone: data.timezone } })
    revalidatePath("/dashboard/automatisations")
    return { success: true as const }
  }, "automation.write")
}
...
export async function enrollLeadInSequence(sequenceId: string, leadId: string) {
  return withAuth(async ({ companyId, userId }) => {
    ...
    await logAction({ userId, action: "ENROLL_EMAIL_SEQUENCE", resource: "EMAIL_SEQUENCE", resourceId: parsedSequenceId, payload: { leadId: parsedLeadId, enrollmentId: enrollment.id } })
    revalidatePath("/dashboard/automatisations")
    return { success: true as const }
  }, "automation.write")
}
...
export async function createAutomationWorkflow(input: unknown) {
  return withAuth(async ({ companyId, userId }) => {
    ...
    await logAction({ userId, action: "CREATE_AUTOMATION_WORKFLOW", resource: "AUTOMATION_WORKFLOW", resourceId: workflow.id, payload: { name: workflow.name, trigger: workflow.trigger } })
    revalidatePath("/dashboard/automatisations")
    return { success: true as const }
  }, "automation.write")
}
...
export async function updateAutomationWorkflow(workflowId: string, input: unknown) {
  return withAuth(async ({ companyId, userId }) => {
    ...
    const configuration = workflowConfigurationSchema.parse({ conditions: parsed.conditions, actions: parsed.actions })
    assertWorkflowCompatibility(parsed.trigger, configuration)
    const workflow = await prisma.automationWorkflow.findFirst({ where: { id, companyId, status: { not: "ARCHIVED" } }, include: { versions: { orderBy: { version: "desc" }, take: 1 } } })
    ...
    await prisma.$transaction(async (tx) => {
      await tx.automationWorkflow.update({ where: { id }, data: { name: parsed.name, trigger: parsed.trigger, conditions: configuration.conditions || undefined, actions: configuration.actions, publishedVersion: null } })
      await tx.automationWorkflowVersion.create({ data: { companyId, workflowId: id, version, status: "DRAFT", trigger: parsed.trigger, conditions: configuration.conditions || undefined, actions: configuration.actions } })
    })
    await logAction({ userId, action: "UPDATE_AUTOMATION_WORKFLOW", resource: "AUTOMATION_WORKFLOW", resourceId: id, payload: { name: parsed.name, trigger: parsed.trigger, version } })
    revalidatePath("/dashboard/automatisations")
...
export async function duplicateAutomationWorkflow(workflowId: string) {
  return withAuth(async ({ companyId, userId }) => {
    ...
    const copy = await prisma.$transaction(async (tx) => {
      const workflow = await tx.automationWorkflow.create({ data: { companyId, name, trigger: source.trigger, conditions: configuration.conditions || undefined, actions: configuration.actions } })
      await tx.automationWorkflowVersion.create({ data: { companyId, workflowId: workflow.id, version: 1, status: "DRAFT", trigger: source.trigger, conditions: configuration.conditions || undefined, actions: configuration.actions } })
      return workflow
    })
    ...
    if (nextStatus === "ACTIVE") {
      ...
      await prisma.$transaction(async (tx) => {
        ...
        if (latest && latestSnapshot === snapshot) {
          ...
          await tx.automationWorkflowVersion.create({ data: { companyId, workflowId: workflow.id, version: publishedVersion, status: "PUBLISHED", trigger: workflow.trigger, conditions: configuration.conditions || undefined, actions: configuration.actions, publishedAt: new Date() } })
        }
