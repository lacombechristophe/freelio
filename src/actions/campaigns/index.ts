...
export async function getCampaignDashboard() {
  return withAuth(async ({ companyId }) => {
    const [campaigns, segments, sequences, members, attributed] = await Promise.all([
      prisma.marketingCampaign.findMany({
        ...
        include: {
          ...
          assets: { orderBy: [{ dueAt: "asc" }, { createdAt: "asc" }] },
          sequences: { include: { deliveries: { select: { status: true } }, _count: { select: { enrollments: true } } }, orderBy: { updatedAt: "desc" } },
        },
        orderBy: [{ startAt: "desc" }, { createdAt: "desc" }],
      }),
      prisma.marketingSegment.findMany({ where: { companyId, status: "ACTIVE" }, select: { id: true, name: true, _count: { select: { memberships: true } } }, orderBy: { name: "asc" } }),
      prisma.emailSequence.findMany({ where: { companyId, status: { not: "ARCHIVED" } }, select: { id: true, name: true, status: true, campaignId: true }, orderBy: { name: "asc" } }),
      prisma.membership.findMany({ where: { companyId, status: "ACTIVE" }, select: { id: true, user: { select: { name: true, email: true } } }, orderBy: { createdAt: "asc" } }),
    ...
    return {
      ...
      campaigns: campaigns.map((campaign) => {
        const deliveryStatuses = campaign.sequences.flatMap((sequence) => sequence.deliveries.map((delivery) => delivery.status))
        return {
          ...campaign,
          ...
          deliveryStats: {
            ...
            delivered: deliveryStatuses.filter((status) => ["DELIVERED", "OPENED", "CLICKED"].includes(status)).length,
            opened: deliveryStatuses.filter((status) => ["OPENED", "CLICKED"].includes(status)).length,
            clicked: deliveryStatuses.filter((status) => status === "CLICKED").length,
            failed: deliveryStatuses.filter((status) => ["FAILED", "BOUNCED", "COMPLAINED", "SUPPRESSED"].includes(status)).length,
          },
        }
      }),
    }
...
export async function createMarketingCampaign(input: unknown) {
  return withAuth(async ({ companyId, userId }) => {
    ...
    const campaign = await prisma.marketingCampaign.create({ data: { companyId, name: data.name, objective: data.objective, channels: data.channels, segmentId: segment?.id || null, ownerMembershipId: owner?.id || null, startAt: data.startAt instanceof Date ? data.startAt : null, endAt: data.endAt instanceof Date ? data.endAt : null, budgetCents: data.budgetCents, utmCampaign: data.utmCampaign || null, notes: data.notes || null } })
    await logAction({ userId, action: "CREATE_MARKETING_CAMPAIGN", resource: "MARKETING_CAMPAIGN", resourceId: campaign.id, payload: { name: campaign.name, channels: data.channels } })
...
const assetSchema = z.object({ campaignId: cuid, type: z.enum(["EMAIL", "FORM", "SMS", "SOCIAL", "ADS", "EVENT", "CONTENT", "DOCUMENT", "OTHER"]), name: z.string().trim().min(2).max(160), ownerMembershipId: z.union([cuid, z.literal("")]).optional(), dueAt: z.union([z.coerce.date(), z.literal(""), z.null()]).optional(), url: z.union([z.string().trim().url(), z.literal("")]).optional() })
...
export async function addMarketingCampaignAsset(input: unknown) {
  return withAuth(async ({ companyId, userId }) => {
    ...
    if (!campaign) throw new Error("Campagne introuvable")
    if (data.ownerMembershipId && !await prisma.membership.findFirst({ where: { id: data.ownerMembershipId, companyId, status: "ACTIVE" }, select: { id: true } })) throw new Error("Responsable introuvable")
    const asset = await prisma.marketingCampaignAsset.create({ data: { campaignId: campaign.id, type: data.type, name: data.name, ownerMembershipId: data.ownerMembershipId || null, dueAt: data.dueAt instanceof Date ? data.dueAt : null, url: data.url || null } })
    await logAction({ userId, action: "UPDATE_MARKETING_CAMPAIGN_ASSET", resource: "MARKETING_CAMPAIGN_ASSET", resourceId: asset.id, payload: { campaignId: campaign.id, type: asset.type } })
    revalidatePath("/dashboard/campagnes")
    return { success: true as const }
  }, "automation.write")
}
