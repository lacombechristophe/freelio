...
export async function submitSignInWithEmail(_previousState: SignInState, formData: FormData): Promise<SignInState> {
...
export async function signInWithEmail(formData: FormData) {
  ...
  try {
    await signIn(method === "magic" ? "resend" : "credentials", { email: email.trim().toLowerCase(), password, mfaCode, redirectTo })
