...
export async function GET(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  await logAction({ userId: access.context.userId, action: "GENERATE_PDF", resource: "DELIVERY_NOTE", resourceId: note.id, payload: { number: note.number } })
  return new NextResponse(new Uint8Array(pdf), { headers: { "content-type": "application/pdf", "content-disposition": `inline; filename="${note.number}.pdf"`, "cache-control": "private, no-store" } })
}
