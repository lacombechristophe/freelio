...
const exportFiltersSchema = z.object({
  days: z.coerce.number().int().refine((value) => [30, 90, 180, 365].includes(value)),
  assignedMembershipId: z.string().cuid().optional(),
  priority: z.enum(["LOW", "NORMAL", "HIGH", "URGENT"]).optional(),
})

export async function GET(request: Request) {
  const access = await getRouteAuth("service.read")
  ...
  const [analytics, company] = await Promise.all([
    getServiceAnalytics(filters),
    prisma.company.findUnique({ where: { id: access.context.companyId }, select: { name: true } }),
  ])
  ...
  const teamRows = analytics.byAssignee.map((item) => ({ Responsable: item.name, Crees: item.created, Clos: item.closed, Backlog: item.backlog, SLAPremiereReponse: item.firstResponsePercent ?? "", SLAResolution: item.resolutionPercent ?? "", ResolutionMoyenneMinutes: item.averageResolutionMinutes ?? "" }))
  const priorityRows = analytics.byPriority.map((item) => ({ Priorite: item.key, Crees: item.created, Clos: item.closed, Backlog: item.backlog, SLAPremiereReponse: item.firstResponsePercent ?? "", SLAResolution: item.resolutionPercent ?? "" }))
  const trendRows = analytics.trend.map((item) => ({ Debut: new Date(item.startAt).toISOString().slice(0, 10), Fin: new Date(item.endAt).toISOString().slice(0, 10), Crees: item.created, Clos: item.closed }))
  ...
  const manifest = {
    ...
    files: fileManifest,
    notes: ["Les SLA incluent les objectifs échus sans réponse ou clôture.", "La satisfaction et la santé sont globales pour la période et ne sont pas filtrées par responsable."],
  }
  const zip = zipSync({
    ...Object.fromEntries(Object.entries(csvFiles).map(([name, content]) => [name, strToU8(`\uFEFF${content}`)])),
    "manifest.json": strToU8(JSON.stringify(manifest, null, 2)),
  }, { level: 6 })
  await logAction({ userId: access.context.userId, action: "EXPORT_SERVICE_ANALYTICS", resource: "SERVICE_ANALYTICS", payload: { days: analytics.filters.days, assignedMembershipId: analytics.filters.assignedMembershipId || null, priority: analytics.filters.priority || null, bytes: zip.byteLength } })
  const stamp = new Date().toISOString().slice(0, 10)
  return new Response(Buffer.from(zip), { headers: { "content-type": "application/zip", "content-disposition": `attachment; filename="${filenameSlug(company.name)}-analyses-service-${stamp}.zip"`, "content-length": String(zip.byteLength), "cache-control": "private, no-store" } })
}
