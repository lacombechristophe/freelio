...
function filenameSlug(value: string) {
  return value.normalize("NFKD").replace(/[\u0300-\u036f]/g, "").toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/^-|-$/g, "").slice(0, 48) || "entreprise"
}
...
export async function GET(request: Request) {
  ...
  const [company, invoices, expenses] = await Promise.all([
    prisma.company.findUnique({ where: { id: access.context.companyId }, select: { name: true, siret: true } }),
    prisma.invoice.findMany({
      where: { companyId: access.context.companyId, status: { in: ["SENT", "OVERDUE", "PAID"] }, date: { gte: from, lt: to } },
      include: { client: { select: { id: true, name: true, siret: true } }, project: { select: { name: true } }, lines: { orderBy: { order: "asc" } }, payments: { orderBy: { date: "asc" } } },
      orderBy: [{ date: "asc" }, { number: "asc" }],
    }),
    prisma.expense.findMany({ where: { companyId: access.context.companyId, date: { gte: from, lt: to } }, include: { client: { select: { name: true } }, project: { select: { name: true } }, files: { select: { name: true, sha256: true } } }, orderBy: [{ date: "asc" }, { id: "asc" }] }),
  ])
  ...
  const lineRows = invoices.flatMap((invoice) => invoice.lines.map((line) => ({ Numero: invoice.number, Libelle: line.label, Description: line.description ?? "", Quantite: line.quantity, PrixUnitaireHT: (line.unitPriceCents / 100).toFixed(2), TauxTVA: line.tvaRate })))
  const paymentRows = invoices.flatMap((invoice) => invoice.payments.map((payment) => ({ NumeroFacture: invoice.number, Client: invoice.client.name, Date: iso(payment.date).slice(0, 10), Montant: (payment.amountCents / 100).toFixed(2), Mode: payment.method, Reference: payment.reference ?? "" })))
  const purchaseRows = expenses.map((expense) => ({ Date: iso(expense.date).slice(0, 10), Fournisseur: expense.provider ?? "", Libelle: expense.label, Categorie: expense.category, MontantTTC: (expense.amountCents / 100).toFixed(2), TVA: (expense.tvaCents / 100).toFixed(2), Client: expense.client?.name ?? "", Projet: expense.project?.name ?? "", Statut: expense.status, Justificatifs: expense.files.map((file) => `${file.name}${file.sha256 ? `#${file.sha256}` : ""}`).join(" | ") }))
  ...
  const fileManifest = Object.fromEntries(Object.entries(csvFiles).map(([name, content]) => [name, { bytes: Buffer.byteLength(content, "utf8"), sha256: createHash("sha256").update(content).digest("hex") }]))
  const manifest = {
  ...
  return new Response(Buffer.from(zip), { headers: { "content-type": "application/zip", "content-disposition": `attachment; filename="${filenameSlug(company.name)}-export-comptable-${stamp}.zip"`, "content-length": String(zip.byteLength), "cache-control": "private, no-store" } })
}
