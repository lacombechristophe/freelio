...
import { AlertTriangle, ArrowRight, Bot, CheckCircle2, CircleDashed, Clock3, LibraryBig, Mail, PhoneCall, Play, Send, Users, Workflow } from "lucide-react"

import { installPoolAutomationPresets, processSequenceEmailsNow } from "@/actions/automations"
...
export function AutomationOverview({ data, pending, run, onNavigate }: { data: AutomationData; pending: boolean; run: AutomationRunner; onNavigate: (tab: string) => void }) {
  ...
  const failures = ["FAILED", "BOUNCED", "COMPLAINED", "SUPPRESSED", "DEAD_LETTER"].reduce((sum, status) => sum + (deliveryCounts[status] ?? 0), 0)
      ...
      <Card className="workspace-panel">
        ...
        <CardContent className="p-0">
          {data.deliveries.length ? <div className="divide-y">{data.deliveries.slice(0, 7).map((delivery) => <div key={delivery.id} className="flex min-w-0 items-center gap-3 px-5 py-3.5">
            <span className={`grid size-8 shrink-0 place-items-center rounded-lg ${["FAILED", "BOUNCED", "COMPLAINED", "SUPPRESSED", "DEAD_LETTER"].includes(delivery.status) ? "bg-destructive/10 text-destructive" : "bg-primary/10 text-primary"}`}><Mail className="size-4" /></span>
          ...
          <div className="border-t bg-muted/20 px-5 py-2"><Button variant="link" className="min-h-9 p-0 text-xs" onClick={() => onNavigate("history")}>Ouvrir le journal complet</Button></div>
        </CardContent>
      </Card>
...
function Stat({ label, value, detail, icon: Icon, tone, className = "" }: { label: string; value: number | string; detail: string; icon: typeof Mail; tone: "blue" | "teal" | "amber" | "red"; className?: string }) {
  ...
  return <article className={`workspace-metric relative min-h-[128px] min-w-0 rounded-xl border bg-card p-4 ${className}`} data-tone={tone}><span className={`absolute right-4 top-4 z-10 grid size-9 place-items-center rounded-lg ${iconTone}`}><Icon className="size-4" /></span><div className="relative z-10 min-w-0 pr-11"><p className="line-clamp-2 text-[13px] font-medium leading-4 text-foreground/85">{label}</p><p className="mt-2.5 text-[25px] font-semibold leading-none tabular-nums tracking-tight">{value}</p></div><p className="relative z-10 mt-4 line-clamp-2 border-t border-border/70 pt-2.5 text-xs leading-4 text-muted-foreground">{detail}</p></article>
}
