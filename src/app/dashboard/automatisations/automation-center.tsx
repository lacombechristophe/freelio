...
import { Activity, Bot, CheckCircle2, Clock3, Eye, FlaskConical, GitBranch, History, Mail, Pause, PhoneCall, Play, Plus, Send, Square, Users } from "lucide-react"
...
const TRIGGER_LABELS: Record<string, string> = { LEAD_CREATED: "Prospect créé", LEAD_STATUS_CHANGED: "Statut prospect modifié", QUOTE_STATUS_CHANGED: "Statut devis modifié", EMAIL_RECEIVED: "E-mail reçu", EMAIL_OPENED: "E-mail ouvert", EMAIL_CLICKED: "Lien d’e-mail cliqué", PORTAL_APPOINTMENT_REQUESTED: "Rendez-vous demandé", INTERVENTION_COMPLETED: "Intervention terminée", CUSTOMER_HEALTH_CHANGED: "Santé client recalculée" }
...
export function AutomationCenter({ initialData }: { initialData: NonNullable<AutomationData> }) {
  ...
  const [simulations, setSimulations] = useState<Record<string, Awaited<ReturnType<typeof simulateAutomationWorkflow>>>>({})
  const [workflowTrigger, setWorkflowTrigger] = useState("LEAD_CREATED")
  const [workflowAction, setWorkflowAction] = useState("ENROLL_SEQUENCE")
    ...
    <div className="space-y-6">
      ...
      <Tabs defaultValue="sequences" className="space-y-5">
            ...
            <Card key={sequence.id}>
              ...
              <CardContent className="grid gap-6 pt-5 xl:grid-cols-[minmax(0,1.25fr)_minmax(320px,0.75fr)]">
                <div className="space-y-4">
                  <details className="rounded-xl border bg-muted/20"><summary className="cursor-pointer list-none px-4 py-3 text-sm font-semibold">Cadence et fenêtre d’envoi <span className="ml-2 text-xs font-normal text-muted-foreground">{sequence.businessDaysOnly ? "jours ouvrés" : "tous les jours"} · {sequence.sendWindowStart} h–{sequence.sendWindowEnd} h · {sequence.timezone}</span></summary><form className="grid gap-3 border-t p-4 sm:grid-cols-2 lg:grid-cols-4" onSubmit={(event) => { event.preventDefault(); const form = new FormData(event.currentTarget); execute(() => updateEmailSequenceSettings({ sequenceId: sequence.id, businessDaysOnly: form.get("businessDaysOnly") === "on", sendWindowStart: formNumber(form, "sendWindowStart"), sendWindowEnd: formNumber(form, "sendWindowEnd"), timezone: form.get("timezone") }), "Cadence de la séquence mise à jour.") }}><label className="flex items-center gap-2 rounded-lg border bg-background px-3 py-2 text-sm"><input name="businessDaysOnly" type="checkbox" defaultChecked={sequence.businessDaysOnly} />Jours ouvrés uniquement</label><Field label="Début"><select name="sendWindowStart" defaultValue={sequence.sendWindowStart} className={controlClass}>{Array.from({ length: 23 }, (_, index) => index).map((hour) => <option key={hour} value={hour}>{hour} h</option>)}</select></Field><Field label="Fin"><select name="sendWindowEnd" defaultValue={sequence.sendWindowEnd} className={controlClass}>{Array.from({ length: 23 }, (_, index) => index + 1).map((hour) => <option key={hour} value={hour}>{hour} h</option>)}</select></Field><Field label="Fuseau"><select name="timezone" defaultValue={sequence.timezone} className={controlClass}><option value="Europe/Paris">Europe/Paris</option><option value="Europe/Brussels">Europe/Bruxelles</option><option value="UTC">UTC</option></select></Field><div className="sm:col-span-2 lg:col-span-4"><Button type="submit" size="sm" variant="outline" disabled={isPending}>Enregistrer la cadence</Button></div></form></details>
        ...
        <TabsContent value="workflows" className="space-y-5">
          <Card>
            <CardHeader><CardTitle className="text-base">Nouvelle règle</CardTitle><CardDescription>Chaque événement possède une clé d’idempotence : une même règle ne s’exécute pas deux fois pour le même changement.</CardDescription></CardHeader>
            <CardContent><form className="grid gap-4 lg:grid-cols-3" onSubmit={(event) => {
              event.preventDefault()
              const form = new FormData(event.currentTarget)
              const actionType = String(form.get("actionType"))
              const action = actionType === "ENROLL_SEQUENCE"
                ? { type: actionType, sequenceId: String(form.get("sequenceId")) }
                : actionType === "CREATE_TASK"
                  ? { type: actionType, title: String(form.get("actionTitle")), delayHours: formNumber(form, "delayHours"), priority: 2 }
                  : actionType === "UPDATE_LEAD_STATUS"
                    ? { type: actionType, status: String(form.get("targetLeadStatus")) }
                    : { type: actionType, title: String(form.get("actionTitle")) }
              const source = String(form.get("source") || "")
              const leadStatus = String(form.get("leadStatus") || "")
              const healthStatus = String(form.get("healthStatus") || "")
              const scoreBelow = String(form.get("healthScoreBelow") || "")
              const scoreDrop = String(form.get("healthScoreDropAtLeast") || "")
              execute(() => createAutomationWorkflow({
                name: form.get("name"),
                ...
                conditions: {
                  ...
                  ...(scoreBelow ? { healthScoreBelow: Number(scoreBelow) } : {}),
                  ...(scoreDrop ? { healthScoreDropAtLeast: Number(scoreDrop) } : {}),
                },
                actions: [action],
              }), "Règle créée en brouillon.", event.currentTarget)
            }}>
              <Field label="Nom"><Input name="name" required placeholder={workflowTrigger === "CUSTOMER_HEALTH_CHANGED" ? "Réagir aux comptes à risque" : "Qualifier les demandes du site"} /></Field>
              <Field label="Déclencheur"><select aria-label="Déclencheur de la règle" name="trigger" value={workflowTrigger} onChange={(event) => { const next = event.target.value; setWorkflowTrigger(next); if (next === "CUSTOMER_HEALTH_CHANGED" && !["CREATE_TASK", "NOTIFY_TEAM"].includes(workflowAction)) setWorkflowAction("CREATE_TASK") }} className={controlClass}>{Object.entries(TRIGGER_LABELS).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></Field>
              {workflowTrigger === "CUSTOMER_HEALTH_CHANGED" ? <>
                <Field label="Niveau de santé"><select aria-label="Niveau de santé client" name="healthStatus" className={controlClass}><option value="">Tous les niveaux</option><option value="RISK">À risque</option><option value="WATCH">À surveiller</option><option value="HEALTHY">Sain</option></select></Field>
                <Field label="Score maximal"><Input aria-label="Score de santé maximal" name="healthScoreBelow" type="number" min="0" max="100" placeholder="49" /></Field>
                <Field label="Baisse minimale"><Input aria-label="Baisse minimale du score" name="healthScoreDropAtLeast" type="number" min="1" max="100" placeholder="10" /></Field>
              </> : <>
                <Field label="Source (facultatif)"><Input name="source" placeholder="WEBSITE" /></Field>
                <Field label="Statut prospect (facultatif)"><select name="leadStatus" className={controlClass}><option value="">Tous</option><option value="NEW">Nouveau</option><option value="CONTACTED">Contacté</option><option value="QUALIFIED">Qualifié</option></select></Field>
              </>}
              <Field label="Action"><select aria-label="Action de la règle" name="actionType" value={workflowAction} onChange={(event) => setWorkflowAction(event.target.value)} className={controlClass}>{workflowTrigger !== "CUSTOMER_HEALTH_CHANGED" && <option value="ENROLL_SEQUENCE">Inscrire dans une séquence</option>}<option value="CREATE_TASK">Créer une tâche</option><option value="NOTIFY_TEAM">Notifier l’équipe</option>{workflowTrigger !== "CUSTOMER_HEALTH_CHANGED" && <option value="UPDATE_LEAD_STATUS">Modifier le statut prospect</option>}</select></Field>
              {workflowAction === "ENROLL_SEQUENCE" && <Field label="Séquence"><select name="sequenceId" className={controlClass}><option value="">Sélectionner…</option>{initialData.sequences.map((sequence) => <option key={sequence.id} value={sequence.id}>{sequence.name}</option>)}</select></Field>}
              {["CREATE_TASK", "NOTIFY_TEAM"].includes(workflowAction) && <Field label="Titre tâche / notification"><Input key={workflowTrigger} aria-label="Titre de l’action automatisée" name="actionTitle" defaultValue={workflowTrigger === "CUSTOMER_HEALTH_CHANGED" ? "Suivre {{client.name}} · santé {{health.score}}/100" : "Suivre {{contact.firstName}} {{contact.lastName}}"} /></Field>}
              {workflowAction === "UPDATE_LEAD_STATUS" && <Field label="Nouveau statut prospect"><select name="targetLeadStatus" className={controlClass}><option value="CONTACTED">Contacté</option><option value="QUALIFIED">Qualifié</option><option value="ARCHIVED">Archivé</option></select></Field>}
              {workflowAction === "CREATE_TASK" && <Field label="Délai tâche (h)"><Input name="delayHours" type="number" min="0" defaultValue="24" /></Field>}
              {workflowTrigger === "CUSTOMER_HEALTH_CHANGED" && <div className="rounded-lg border bg-muted/25 p-3 text-xs leading-5 text-muted-foreground lg:col-span-2">Variables : <code>{"{{client.name}}"}</code>, <code>{"{{health.score}}"}</code>, <code>{"{{health.previousScore}}"}</code> et <code>{"{{health.status}}"}</code>.</div>}
              <div className="flex items-end"><Button type="submit" disabled={isPending}><Bot />Créer la règle</Button></div>
            </form></CardContent>
          </Card>
          <Card><CardHeader><div className="flex items-center gap-2"><GitBranch className="size-4 text-primary" /><CardTitle className="text-base">Nouvelle branche conditionnelle</CardTitle></div><CardDescription>Choisissez le chemin vrai et le chemin alternatif. La simulation permet de vérifier le résultat avant activation.</CardDescription></CardHeader><CardContent><form className="grid gap-4 lg:grid-cols-3" onSubmit={(event) => { event.preventDefault(); const form = new FormData(event.currentTarget); const field = String(form.get("branchField")); const value = String(form.get("branchValue") || ""); const conditions = field === "marketingOptIn" ? { marketingOptIn: value === "true" } : { [field]: value }; const falseType = String(form.get("falseType")); const falseAction = falseType === "NOTIFY_TEAM" ? { type: "NOTIFY_TEAM", title: String(form.get("falseTitle")) } : { type: "CREATE_TASK", title: String(form.get("falseTitle")), delayHours: formNumber(form, "falseDelayHours"), priority: 2 }; execute(() => createAutomationWorkflow({ name: form.get("name"), trigger: form.get("trigger"), actions: [{ type: "CONDITIONAL_BRANCH", label: String(form.get("branchLabel")), conditions, ifTrue: [{ type: "UPDATE_LEAD_STATUS", status: form.get("trueStatus") }], ifFalse: [falseAction] }] }), "Workflow conditionnel créé en brouillon.", event.currentTarget) }}><Field label="Nom"><Input name="name" required placeholder="Qualifier les projets prioritaires" /></Field><Field label="Déclencheur"><select name="trigger" className={controlClass}>{Object.entries(TRIGGER_LABELS).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></Field><Field label="Nom de la branche"><Input name="branchLabel" required defaultValue="Critère de qualification" /></Field><Field label="Critère"><select name="branchField" className={controlClass}><option value="projectTypeContains">Type de projet contient</option><option value="source">Source égale</option><option value="leadStatus">Statut prospect égal</option><option value="marketingOptIn">Consentement marketing égal</option></select></Field><Field label="Valeur"><Input name="branchValue" required defaultValue="couverture" /></Field><Field label="Chemin vrai"><select name="trueStatus" className={controlClass}><option value="QUALIFIED">Qualifier le prospect</option><option value="CONTACTED">Marquer contacté</option><option value="ARCHIVED">Archiver</option></select></Field><Field label="Chemin alternatif"><select name="falseType" className={controlClass}><option value="CREATE_TASK">Créer une tâche</option><option value="NOTIFY_TEAM">Notifier l’équipe</option></select></Field><Field label="Titre alternatif"><Input name="falseTitle" required defaultValue="Vérifier le besoin de {{contact.firstName}}" /></Field><Field label="Délai alternatif (h)"><Input name="falseDelayHours" type="number" min="0" max="8760" defaultValue="4" /></Field><div className="lg:col-span-3"><Button type="submit" variant="outline" disabled={isPending}><GitBranch />Créer la branche</Button></div></form></CardContent></Card>
          <div className="grid gap-4 lg:grid-cols-2">{initialData.workflows.map((workflow) => <Card key={workflow.id}><CardHeader><div className="flex items-start justify-between gap-3"><div><CardTitle className="text-sm">{workflow.name}</CardTitle><CardDescription>{TRIGGER_LABELS[workflow.trigger] ?? workflow.trigger}</CardDescription></div><Badge variant={workflow.status === "ACTIVE" ? "default" : "outline"}>{STATUS_LABELS[workflow.status] ?? workflow.status}</Badge></div></CardHeader><CardContent className="space-y-4"><div className="flex flex-wrap gap-x-4 gap-y-1 text-xs text-muted-foreground"><span>{workflow.runs.length} exécution(s) récente(s)</span><span>{workflow.runs.filter((run) => run.status === "FAILED").length} échec(s)</span><span className="inline-flex items-center gap-1"><History className="size-3.5" />{workflow.versions.length} version(s)</span>{workflow.publishedVersion && <span>Version publiée v{workflow.publishedVersion}</span>}</div><div className="flex gap-2">{workflow.status === "ACTIVE" ? <Button size="sm" variant="outline" onClick={() => execute(() => updateAutomationWorkflowStatus(workflow.id, "PAUSED"), "Règle mise en pause.")}><Pause />Pause</Button> : <Button size="sm" onClick={() => execute(() => updateAutomationWorkflowStatus(workflow.id, "ACTIVE"), "Règle activée.")}><Play />Publier et activer</Button>}</div><form className="space-y-2 border-t pt-4" onSubmit={(event) => { event.preventDefault(); const subjectId = String(new FormData(event.currentTarget).get("subjectId")); startTransition(async () => { try { const result = await simulateAutomationWorkflow(workflow.id, subjectId); setSimulations((current) => ({ ...current, [workflow.id]: result })); toast.success("Simulation terminée sans effet.") } catch (error) { toast.error(error instanceof Error ? error.message : "Simulation impossible") } }) }}><Field label="Tester sans exécuter"><select name="subjectId" className={controlClass} required><option value="">{workflow.trigger === "CUSTOMER_HEALTH_CHANGED" ? "Choisir un client…" : "Choisir un prospect…"}</option>{workflow.trigger === "CUSTOMER_HEALTH_CHANGED" ? initialData.clients.map((client) => <option key={client.id} value={client.id}>{client.name} · {client.score}/100</option>) : initialData.leads.map((lead) => <option key={lead.id} value={lead.id}>{lead.firstName} {lead.lastName} · {lead.projectType || lead.source}</option>)}</select></Field><Button type="submit" size="sm" variant="outline" disabled={isPending}><FlaskConical />Simuler</Button></form>{simulations[workflow.id] && <div className="rounded-lg bg-muted/45 p-3 text-xs"><p className="font-semibold">{simulations[workflow.id].matches ? "Conditions remplies" : "Conditions non remplies"} · {simulations[workflow.id].lead}</p><div className="mt-2 space-y-1 text-muted-foreground">{simulations[workflow.id].trace.map((item, index) => <p key={`${item.type}-${index}`}>{item.type === "BRANCH" ? `${item.label} → branche ${item.selected === "TRUE" ? "vraie" : "alternative"}` : `${item.label} → ${item.matched ? "oui" : "non"}`}</p>)}<p>Actions prévues : {simulations[workflow.id].actions.map((item) => item.type).join(", ") || "aucune"}</p></div></div>}<details className="border-t pt-3"><summary className="cursor-pointer text-xs font-medium">Historique des versions</summary><div className="mt-2 space-y-1">{workflow.versions.map((version) => <p key={version.id} className="flex justify-between text-xs text-muted-foreground"><span>v{version.version} · {version.status}</span><span>{version.publishedAt ? new Intl.DateTimeFormat("fr-FR", { dateStyle: "short", timeStyle: "short" }).format(new Date(version.publishedAt)) : "Brouillon"}</span></p>)}</div></details></CardContent></Card>)}{!initialData.workflows.length ? <p className="rounded-xl border border-dashed p-8 text-center text-sm text-muted-foreground lg:col-span-2">Aucune règle.</p> : null}</div>
        </TabsContent>
