...
import { Activity, Bot, CheckCircle2, Clock3, Eye, Mail, Pause, Play, Plus, Send, Square, Users } from "lucide-react"
            ...
            <Card key={sequence.id}>
              ...
              <CardContent className="grid gap-6 pt-5 xl:grid-cols-[minmax(0,1.25fr)_minmax(320px,0.75fr)]">
                <div className="space-y-4">
                  ...
                  {sequence.steps.length ? <div className="space-y-2">{sequence.steps.map((step, index) => <div key={step.id} className="rounded-xl border p-4"><div className="flex items-center gap-2"><span className="grid size-7 place-items-center rounded-lg bg-primary/10 text-xs font-semibold text-primary">{index + 1}</span><p className="font-medium">{step.subject}</p><Badge className="ml-auto" variant="outline"><Clock3 />{step.delayHours ? `${step.delayHours} h après` : "Immédiat"}</Badge></div><div className="mt-3 line-clamp-2 text-xs leading-5 text-muted-foreground" dangerouslySetInnerHTML={{ __html: step.bodyHtml.replace(/<[^>]+>/g, " ") }} /><StepPerformance deliveries={step.deliveries} /></div>)}</div> : <p className="rounded-xl border border-dashed p-5 text-sm text-muted-foreground">Aucune étape.</p>}
