            ...
            <Card key={sequence.id}>
              ...
              <CardContent className="grid gap-6 pt-5 xl:grid-cols-[minmax(0,1.25fr)_minmax(320px,0.75fr)]">
                ...
                <div className="space-y-4">
                  ...
                  {sequence.enrollments.length ? <div className="divide-y rounded-xl border">{sequence.enrollments.map((item) => <div key={item.id} className="flex items-center gap-3 p-3"><div className="min-w-0 flex-1"><p className="truncate text-sm font-medium">{item.leadCapture.firstName} {item.leadCapture.lastName}</p><p className="truncate text-xs text-muted-foreground">{item.leadCapture.email} · {STATUS_LABELS[item.status] ?? item.status}</p>{item.taskExecutions[0] && <p className="mt-1 text-[11px] text-muted-foreground">Dernière tâche : {item.taskExecutions[0].organisationTask.title} · {item.taskExecutions[0].organisationTask.status === "DONE" ? "terminée" : "à faire"}</p>}</div>{item.status === "ACTIVE" ? <Button size="icon-sm" variant="ghost" title="Arrêter" onClick={() => execute(() => stopSequenceEnrollment(item.id), "Inscription arrêtée.")}><Square /></Button> : null}</div>)}</div> : <p className="text-sm text-muted-foreground">Aucune inscription récente.</p>}
                </div>
              </CardContent>
            </Card>
