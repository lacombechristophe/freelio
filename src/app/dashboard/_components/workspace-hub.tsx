...
export function WorkspaceHub({ eyebrow, title, description, metrics, featured, panels, sections, primaryAction }: { eyebrow: string; title: string; description: string; metrics: WorkspaceMetric[]; featured?: ReactNode; panels?: WorkspacePanel[]; sections: WorkspaceSection[]; primaryAction?: WorkspaceLink }) {
  return <div className="space-y-4.5">
    <PageHeader eyebrow={eyebrow} title={title} description={description} actions={primaryAction ? <Button nativeButton={false} render={<Link href={primaryAction.href} />}><primaryAction.icon />{primaryAction.name}</Button> : undefined} />

    <section aria-label="Indicateurs" className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
  ...
  return (
    <section className="overflow-hidden rounded-xl border bg-card shadow-[0_1px_2px_rgba(13,36,66,0.035)]">
      <header className="flex items-start justify-between gap-4 border-b px-4.5 py-3.5">
        <div><h2 className="text-sm font-semibold">{panel.title}</h2><p className="mt-0.5 text-xs text-muted-foreground">{panel.description}</p></div>
      ...
      {panel.rows.length ? <div className="divide-y">{panel.rows.map((row) => {
        ...
        return <Link key={`${row.href}-${row.title}`} href={row.href} className="group flex min-h-[62px] items-center gap-3 px-4.5 py-2.5 transition-colors hover:bg-muted/35"><span className={`grid size-8 shrink-0 place-items-center rounded-lg ${tones}`}><Icon className="size-3.5" /></span><span className="min-w-0 flex-1"><span className="block truncate text-sm font-semibold">{row.title}</span><span className="mt-0.5 block truncate text-xs text-muted-foreground">{row.detail}</span></span>{row.meta ? <span className="hidden max-w-32 truncate text-right text-xs text-muted-foreground sm:block">{row.meta}</span> : null}{row.status ? <Badge variant={tone === "red" ? "destructive" : "outline"} className="shrink-0">{row.status}</Badge> : null}<ArrowRight className="size-3.5 shrink-0 text-muted-foreground/60 transition-transform group-hover:translate-x-0.5" /></Link>
      })}</div> : <p className="px-4.5 py-10 text-center text-sm text-muted-foreground">{panel.empty}</p>}
      <Link href={panel.href} className="flex h-10 items-center justify-center gap-1.5 border-t text-xs font-semibold text-primary transition-colors hover:bg-primary/5">{panel.linkLabel}<ArrowRight className="size-3.5" /></Link>
    </section>
  )
}
...
function WorkspaceMetricCard({ metric }: { metric: WorkspaceMetric }) {
  ...
  return (
    <article className="min-h-[128px] rounded-xl border bg-card p-4 shadow-[0_1px_2px_rgba(13,36,66,0.035)]">
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0">
          <p className="truncate text-xs font-medium text-foreground/85">{metric.label}</p>
          <p className="mt-2 text-[22px] font-semibold leading-none tabular-nums text-foreground">{metric.value}</p>
        </div>
