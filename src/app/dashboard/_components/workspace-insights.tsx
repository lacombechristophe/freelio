  ...
  return (
    <section className="workspace-trend overflow-hidden rounded-xl border bg-card">
      ...
      <div className="px-4.5 py-3.5">
        ...
        ) : <div className="workspace-empty"><div><CircleDot className="mx-auto size-5 text-muted-foreground/60" /><p className="mt-2 text-sm font-medium">Pas encore assez d’historique</p><p className="mt-1 text-xs text-muted-foreground">Le graphique apparaîtra dès que l’activité sera enregistrée.</p></div></div>}
      </div>
      <Link href={href} className="workspace-link flex items-center justify-center gap-1.5 border-t text-[13px] font-semibold text-primary transition-colors hover:bg-primary/5">{linkLabel}<ArrowRight className="size-3.5" /></Link>
    </section>
  )
}
...
export function WorkspaceDistributionPanel({ title, description, items, href, linkLabel }: { title: string; description: string; items: Array<{ label: string; value: number; detail?: string }>; href: string; linkLabel: string }) {
  ...
  return <section className="workspace-distribution overflow-hidden rounded-xl border bg-card"><header className="border-b px-4.5 py-3.5"><h2 className="text-[15px] font-semibold">{title}</h2><p className="mt-0.5 text-[13px] text-muted-foreground">{description}</p></header>{items.length ? <div className="space-y-4 px-4.5 py-4">{items.map((item, index) => <div key={item.label}><div className="mb-1.5 flex items-center justify-between gap-3 text-[13px]"><span className="truncate font-medium">{item.label}</span><span className="shrink-0 tabular-nums text-muted-foreground">{item.value} {total ? `· ${Math.round(item.value / total * 100)} %` : ""}</span></div><div className="h-2 overflow-hidden rounded-full bg-muted"><div className="h-full rounded-full" style={{ width: `${item.value / max * 100}%`, backgroundColor: CHART_COLORS[index % CHART_COLORS.length] }} /></div>{item.detail ? <p className="mt-1 text-xs text-muted-foreground">{item.detail}</p> : null}</div>)}</div> : <p className="workspace-empty text-sm text-muted-foreground">Aucune donnée disponible.</p>}<Link href={href} className="workspace-link flex items-center justify-center gap-1.5 border-t text-[13px] font-semibold text-primary transition-colors hover:bg-primary/5">{linkLabel}<ArrowRight className="size-3.5" /></Link></section>
}
