...
import { Link2, Mail, MessageSquareText, Plus, Send, StickyNote } from "lucide-react"
...
export function ServiceConversationPanel({ ticket }: { ticket: Ticket }) {
  const router = useRouter()
  const [pending, startTransition] = React.useTransition()
  const contacts = ticket.client.contacts.filter((contact) => contact.email)
  const latestThread = ticket.emailThreads.at(-1)
  const timeline = [
    ...ticket.notes.map((note) => ({ id: note.id, kind: "NOTE" as const, at: note.createdAt, body: note.body, author: note.authorMembership?.user.name || note.authorMembership?.user.email || "Équipe" })),
    ...ticket.emailThreads.flatMap((thread) => thread.messages.map((message) => ({ id: message.id, kind: "EMAIL" as const, at: message.receivedAt || message.sentAt || message.createdAt, body: message.bodyText || plainText(message.bodyHtml), author: message.direction === "INBOUND" ? message.fromAddress : "Équipe", direction: message.direction, subject: message.subject, status: message.status, events: message.events.map((event) => event.type) }))),
  ...
  return <Card>
    <CardHeader><div className="flex items-start justify-between gap-3"><div><CardTitle className="flex items-center gap-2 text-base"><MessageSquareText className="size-4 text-primary" />Conversation et notes</CardTitle><CardDescription className="mt-1">Les e-mails client et les notes internes restent dans une chronologie unique, avec une nature clairement distincte.</CardDescription></div><HelpTip label="Différence entre e-mail et note">Un e-mail est envoyé au client via le canal configuré. Une note interne n’est visible que par l’équipe.</HelpTip></div></CardHeader>
    <CardContent className="space-y-5">
      {ticket.client.emailThreads.length > 0 && <form className="flex flex-col gap-2 rounded-lg border bg-muted/20 p-3 sm:flex-row" onSubmit={(event) => { event.preventDefault(); const form = event.currentTarget; run(() => linkServiceTicketThread({ ticketId: ticket.id, threadId: new FormData(form).get("threadId") }), "Conversation rattachée au ticket.", form) }}><select name="threadId" required aria-label="Conversation client à rattacher" className={`${controlClass} flex-1`}><option value="">Rattacher une conversation existante…</option>{ticket.client.emailThreads.map((thread) => <option key={thread.id} value={thread.id}>{thread.subject} · {date(thread.lastMessageAt)}</option>)}</select><Button type="submit" size="sm" variant="outline" disabled={pending}><Link2 />Rattacher</Button></form>}

      <div className="max-h-[520px] space-y-3 overflow-y-auto rounded-xl border bg-muted/15 p-3">
        {timeline.length ? timeline.map((item) => item.kind === "NOTE" ? <article key={`note-${item.id}`} className="mr-4 rounded-lg border bg-amber-50/70 p-3 text-amber-950 dark:bg-amber-950/20 dark:text-amber-100"><div className="flex flex-wrap items-center gap-2"><StickyNote className="size-3.5" /><Badge variant="outline">Note interne</Badge><span className="text-xs font-medium">{item.author}</span><span className="ml-auto text-[11px] opacity-70">{date(item.at)}</span></div><p className="mt-2 whitespace-pre-wrap text-sm leading-6">{item.body}</p></article> : <article key={`email-${item.id}`} className={`rounded-lg border bg-background p-3 ${item.direction === "OUTBOUND" ? "ml-4" : "mr-4"}`}><div className="flex flex-wrap items-center gap-2"><Mail className="size-3.5 text-primary" /><Badge variant={item.direction === "INBOUND" ? "secondary" : "outline"}>{item.direction === "INBOUND" ? "Client → équipe" : "Équipe → client"}</Badge><span className="truncate text-xs font-medium">{item.author}</span><span className="ml-auto text-[11px] text-muted-foreground">{date(item.at)}</span></div><p className="mt-2 text-sm font-semibold">{item.subject}</p><p className="mt-1 whitespace-pre-wrap text-sm leading-6 text-muted-foreground">{item.body || "Message HTML sans version texte."}</p><p className="mt-2 text-[11px] text-muted-foreground">{item.status}{item.events.length ? ` · ${item.events.at(-1)}` : ""}</p></article>) : <p className="py-10 text-center text-sm text-muted-foreground">Aucun échange rattaché. Répondez au client ou rattachez une conversation existante.</p>}
      </div>

      <div className="grid gap-4 lg:grid-cols-2">
        <form key={`reply-${latestThread?.id || "new"}`} className="space-y-3 rounded-xl border p-4" onSubmit={(event) => { event.preventDefault(); const form = event.currentTarget; const data = new FormData(form); const body = String(data.get("body") || ""); run(() => sendCrmEmail({ serviceTicketId: ticket.id, threadId: data.get("threadId"), contactId: data.get("contactId"), subject: data.get("subject"), bodyHtml: `<p>${escapeHtml(body).replace(/\r?\n/g, "<br>")}</p>` }), "Réponse envoyée et rattachée au ticket.", form) }}><div className="flex items-center gap-2"><h3 className="text-sm font-semibold">Répondre au client</h3><HelpTip label="Canal d’envoi">L’envoi exige un canal Resend actif. Google et Microsoft seront proposés après autorisation OAuth.</HelpTip></div><div><Label htmlFor={`ticket-contact-${ticket.id}`}>Destinataire</Label><select id={`ticket-contact-${ticket.id}`} name="contactId" required defaultValue={contacts[0]?.id || ""} className={`${controlClass} mt-1.5`}><option value="">Choisir un contact…</option>{contacts.map((contact) => <option key={contact.id} value={contact.id}>{contact.firstName} {contact.lastName} · {contact.email}</option>)}</select></div><div><Label htmlFor={`ticket-thread-${ticket.id}`}>Conversation</Label><select id={`ticket-thread-${ticket.id}`} name="threadId" defaultValue={latestThread?.id || ""} className={`${controlClass} mt-1.5`}><option value="">Nouvelle conversation</option>{ticket.emailThreads.map((thread) => <option key={thread.id} value={thread.id}>{thread.subject}</option>)}</select></div><div><Label htmlFor={`ticket-subject-${ticket.id}`}>Objet</Label><Input id={`ticket-subject-${ticket.id}`} name="subject" className="mt-1.5" required defaultValue={latestThread ? `Re: ${latestThread.subject}` : `${ticket.number} · ${ticket.title}`} /></div><div><Label htmlFor={`ticket-body-${ticket.id}`}>Message</Label><Textarea id={`ticket-body-${ticket.id}`} name="body" className="mt-1.5" rows={5} required minLength={3} placeholder="Bonjour, voici la mise à jour de votre demande…" /></div><Button type="submit" disabled={pending || contacts.length === 0}><Send />Envoyer la réponse</Button>{contacts.length === 0 && <p className="text-xs text-danger">Ajoutez une adresse e-mail à un contact client avant de répondre.</p>}</form>
