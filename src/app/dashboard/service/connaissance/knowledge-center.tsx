...
export function KnowledgeCenter({ initialData }: { initialData: ServiceContentData }) {
  ...
  return <div className="space-y-6">
    <section className="grid overflow-hidden rounded-xl border bg-card sm:grid-cols-3"><Metric icon={BookOpen} label="Articles actifs" value={initialData.articles.length} detail="Hors archives" /><Metric icon={ShieldCheck} label="Validés" value={published} detail="Contenu publié" /><Metric icon={Globe2} label="Espace client" value={portal} detail="Visibles par les clients" /></section>
