...
test("service records connect the help desk, conversation, ticket, intervention and installed equipment", async ({ page }, testInfo) => {
  if (testInfo.project.name === "desktop") {
    ...
    await page.getByLabel("Seuil de la règle").fill("2")
    ...
    await expect(page.getByText("Volume SAV récent QA")).toBeVisible()
...
test("configures an email sequence and a lead automation", async ({ page }, testInfo) => {
  ...
  await publishedWorkflow.locator('select[name="leadId"]').selectOption({ label: "Camille Piscine · Couverture QA" })
