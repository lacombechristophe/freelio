...
test("configures an email sequence and a lead automation", async ({ page }, testInfo) => {
  ...
  const cadence = sequenceCard.locator("details").filter({ hasText: "Cadence et fenêtre d’envoi" })
  ...
  await expect(page.locator("[data-slot=card]").filter({ hasText: "Nurturing QA" }).last().getByText(/tous les jours · 9 h–17 h · UTC/)).toBeVisible()
  ...
  await expect(page.getByText("Livrés").first()).toBeVisible()
  ...
  await expect(page.getByText("Règle activée.").last()).toBeVisible()

  await page.getByRole("tab", { name: "Séquences" }).click()
  const taskSequenceForm = sequencePanel.locator("form").first()
  await taskSequenceForm.locator('input[name="name"]').fill("Relance téléphonique QA")
  await taskSequenceForm.locator('input[name="description"]').fill("Appel obligatoire avant fin de séquence")
  await taskSequenceForm.getByRole("button", { name: "Créer" }).click()
  await expect(page.getByText("Séquence créée.").last()).toBeVisible()
  const taskSequence = page.locator("[data-slot=card]").filter({ hasText: "Relance téléphonique QA" }).last()
  ...
  await expect(page.getByText("Cadence de la séquence mise à jour.").last()).toBeVisible()
  await taskSequence.locator('select[name="type"]').selectOption("CALL_TASK")
  await taskSequence.locator('input[name="delayHours"]').fill("0")
  await taskSequence.locator('input[name="taskTitle"]').fill("Appeler {{contact.firstName}} pour confirmer le besoin QA")
  await taskSequence.locator('textarea[name="taskNotes"]').fill("Valider les dimensions et la date de visite.")
  ...
  await taskSequence.getByRole("button", { name: "Mettre en pause Camille Piscine" }).click()
  await expect(page.getByText("Inscription mise en pause.")).toBeVisible()
  const pausedTaskSequence = page.locator("[data-slot=card]").filter({ hasText: "Relance téléphonique QA" }).last()
  await pausedTaskSequence.getByRole("button", { name: "Reprendre Camille Piscine" }).click()
  await expect(page.getByText("Inscription reprise.")).toBeVisible()
  await page.getByRole("tab", { name: "Journal" }).click()
  await page.getByRole("button", { name: "Exécuter" }).click()
  await expect(page.getByText("Traitement terminé.")).toBeVisible()
  ...
  const completedTaskSequence = page.locator("[data-slot=card]").filter({ hasText: "Relance téléphonique QA" }).last()
  await expect(completedTaskSequence.getByText("Camille Piscine", { exact: true })).toBeVisible()
  await expect(completedTaskSequence.getByText(/Terminée/)).toBeVisible()

  await page.getByRole("tab", { name: "Règles" }).click()
  const branchBuilder = page.locator("[data-slot=card]").filter({ hasText: "Nouvelle branche conditionnelle" })
  await branchBuilder.locator('input[name="name"]').fill("Qualification conditionnelle QA")
  await branchBuilder.locator('select[name="trigger"]').selectOption("EMAIL_CLICKED")
  await branchBuilder.locator('input[name="branchLabel"]').fill("Projet couverture QA")
  await branchBuilder.locator('select[name="branchField"]').selectOption("projectTypeContains")
  await branchBuilder.locator('input[name="branchValue"]').fill("couverture")
  await branchBuilder.locator('select[name="trueStatus"]').selectOption("QUALIFIED")
  await branchBuilder.getByRole("button", { name: "Créer la branche" }).click()
  await expect(page.getByText("Workflow conditionnel créé en brouillon.")).toBeVisible()
  const conditionalWorkflow = page.locator("[data-slot=card]").filter({ hasText: "Qualification conditionnelle QA" }).last()
  await conditionalWorkflow.getByRole("button", { name: "Publier et activer" }).click()
  await expect(page.getByText("Règle activée.").last()).toBeVisible()
  const publishedWorkflow = page.locator("[data-slot=card]").filter({ hasText: "Qualification conditionnelle QA" }).last()
  await expect(publishedWorkflow.getByText("Version publiée v1")).toBeVisible()
  await publishedWorkflow.locator('select[name="subjectId"]').selectOption({ label: "Camille Piscine · Couverture QA" })
  await publishedWorkflow.getByRole("button", { name: "Simuler" }).click()
  await expect(page.getByText("Simulation terminée sans effet.")).toBeVisible()
  await expect(publishedWorkflow.getByText("Projet couverture QA → branche vraie")).toBeVisible()
  await expect(publishedWorkflow.getByText(/Actions prévues : UPDATE_LEAD_STATUS/)).toBeVisible()

  const proactiveBuilder = page.locator("[data-slot=card]").filter({ hasText: "Nouvelle règle" }).first()
  await proactiveBuilder.getByLabel("Déclencheur de la règle").selectOption("CUSTOMER_HEALTH_CHANGED")
  await proactiveBuilder.locator('input[name="name"]').fill("Alerte santé client QA")
  await proactiveBuilder.getByLabel("Score de santé maximal").fill("99")
  await proactiveBuilder.getByLabel("Action de la règle").selectOption("CREATE_TASK")
  await proactiveBuilder.getByLabel("Titre de l’action automatisée").fill("Suivre {{client.name}} · santé {{health.score}}/100")
  await proactiveBuilder.locator('input[name="delayHours"]').fill("0")
  await proactiveBuilder.getByRole("button", { name: "Créer la règle" }).click()
  await expect(page.getByText("Règle créée en brouillon.").last()).toBeVisible()
  const proactiveWorkflow = page.locator("[data-slot=card]").filter({ hasText: "Alerte santé client QA" }).last()
  await proactiveWorkflow.getByRole("button", { name: "Publier et activer" }).click()
  await expect(page.getByText("Règle activée.").last()).toBeVisible()
