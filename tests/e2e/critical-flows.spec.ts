...
test("field report and maintenance contract flow", async ({ page }, testInfo) => {
  ...
  await expect(page.getByText("Lien de signature sécurisé copié.")).toBeVisible()
  const signingUrl = await page.evaluate(() => navigator.clipboard.readText())
...
test("creates a structured amendment without altering the signed source contract", async ({ page }, testInfo) => {
  ...
  await page.getByRole("button", { name: "Actions" }).click()
  await page.getByRole("menuitem", { name: "Marquer comme signé" }).click()
  await expect(page.getByText("Statut mis à jour.")).toBeVisible()
