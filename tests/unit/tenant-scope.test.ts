...
describe("tenant scope architecture", () => {
  it("scopes every model that owns a direct companyId", () => {
    ...
    const expected = directCompanyModels(schema)
      .filter((model) => !COMPANY_SCOPE_SCHEMA_EXCEPTIONS.has(model as "User"))
  ...
  it("classifies every schema model as direct, relational or intentionally global", () => {
    ...
    const classified = new Set([
      ...
      ...COMPANY_RELATION_SCOPED_MODELS,
      ...GLOBAL_SCOPE_SCHEMA_EXCEPTIONS,
    ])
