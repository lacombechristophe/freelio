...
| Modèles, séquences, workflows | **Partiel** | modèles HTML assainis avec aperçu, e-mails automatiques/manuels, appels et tâches, pause jusqu’à réalisation dans Organisation, délais, jours ouvrés, fenêtre/fuseau d’envoi, statistiques par étape, inscriptions consenties, pause/arrêt, arrêt sur réponse ou opposition, triggers lead/devis/e-mail/portail/intervention, notifications, changement de statut et journal idempotent | pas encore de version publiée, d’éditeur graphique, de branches imbriquées ou de test A/B |
...
| PostgreSQL | **Disponible** | schéma miroir, client dédié et vingt-et-une migrations versionnées ; réparation idempotente documentée pour l’ancien schéma Vercel | hébergeur, haute disponibilité, sauvegardes et restauration à mettre en service |
...
| CI/CD | **Partiel** | workflow GitHub Actions : génération Prisma, types, lint, 115 tests unitaires, build et Playwright sur base isolée ; déclenchement push, PR ou manuel | infrastructure externe et secrets de production restent à administrer |
