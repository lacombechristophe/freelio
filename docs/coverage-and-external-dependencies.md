...
| PostgreSQL | **Disponible** | schéma miroir, client dédié et 34 migrations versionnées ; réparation idempotente documentée pour l’ancien schéma Vercel | hébergeur, haute disponibilité, sauvegardes et restauration à mettre en service |
...
| CI/CD | **Partiel** | workflow GitHub Actions : génération Prisma, types, lint, 190 tests unitaires, build et Playwright sur base isolée ; déclenchement push, PR ou manuel | fournisseurs externes et secrets associés restent à administrer |
