...
| Chronologie commerciale | **Partiel** | activités manuelles/importées, conversations e-mail et réunions Google/Microsoft importées dans les tâches de la société | appels externes et rattachement automatique d’une réunion distante à toutes les fiches CRM concernées restent à approfondir |
| Tâches et agenda | **Partiel** | objectifs, tâches, récurrence, heure et fuseau société, export ICS, OAuth Google/Microsoft, création et suppression distantes, import incrémental/delta idempotent, annulations et curseurs chiffrés | invitations avec suivi RSVP, disponibilités/free-busy et rappels fournisseur restent à approfondir ; consentement OAuth réel requis |
...
**Conclusion HubSpot :** le cœur CRM/vente, la capture, les boîtes et calendriers Resend/Google/Microsoft, les séquences, l’activation contrôlée des audiences, le scoring, les segments et les règles événementielles sont couverts dans le produit. Une résiliation reste bloquée tant que les boîtes et calendriers historiques ainsi que leurs consentements OAuth ne sont pas recettés, ou si l’entreprise dépend d’invitations/free-busy avancés, de campagnes à très gros volume, du live chat, de publicité/social ou de workflows à branches non reconstruits.
...
| Devis | **Disponible** | versions, sections, lignes libres ou configurées, options contrôlées serveur, nomenclature/coût, remises, marges par catégorie, TVA mixte et PDF ; transitions de statut strictes et horodatées ; un lancement guidé et rejouable crée la commande, le chantier, l’acompte prévu et le contrat métier sans double facturation ; calcul central pur et testé au centime | formules dimensionnelles propres aux fabricants à paramétrer lorsque leurs barèmes sont disponibles |
...
| Files de travaux | **Disponible sous configuration** | BullMQ pour les documents ; processeurs de séquences, échéances et synchronisations protégés par bail PostgreSQL ; retries temporisés, reprise des brouillons OAuth, file d’échecs terminaux et rejeu humain audité ; workflow GitHub Actions toutes les cinq minutes compatible Vercel Hobby | observer le workflow réel, relier les alertes à un humain et dimensionner quotas/temps d’exécution sur les volumes réels |
...
| CI/CD | **Partiel** | workflow GitHub Actions : génération Prisma, types stricts, lint, audit npm dès le niveau modéré, tests unitaires, seuils de couverture sur finance/imports/Factur-X, build et Playwright sur SQLite isolé ; un second job applique les migrations et rejoue les tests sur PostgreSQL 16 ; un workflow distinct pilote les processeurs de production | les comptes fournisseurs, leurs secrets et la restauration d’infrastructure restent à administrer ; chaque workflow doit être observé sur GitHub Actions après ce déploiement |
